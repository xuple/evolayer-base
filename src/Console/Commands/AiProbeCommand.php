...
class AiProbeCommand extends Command
{
    ...
    {
        // The all-providers path passes $model = null for every provider
        // (OpenCode resolves its default model inside the probe). The service
        // requires a model to persist (it is part of the ledger unique key),
        // so --persist here only records OpenCode rows today — a deliberate
        // choice, not a silent bail. Enabling persistence for the other
    ...
    /**
     * Thin delegate over the shared AiCapabilityProbe service (ADR-019).
     * The cooldown, provider override, exception classification,
     * output_mode detection, conditions synthesis, and ledger write all
     * live in the service so the three probe commands share one
     * implementation. Returns the legacy array{ok,message} so this
     * command's callers (runSingleProvider, runAllModels, runReprobeStale,
     * runAllProviders) are untouched.
     *
     * @return array{ok: bool, message: string}
     */
    private function runProbe(string $agentClass, string $provider, ?string $model, bool $persist): array
    {
        return app(AiCapabilityProbe::class)
            ->probeAndRecord($agentClass, $provider, $model, $persist, (bool) $this->option('force'))
            ->toLegacyArray();
    }
}
